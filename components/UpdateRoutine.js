...
const UpdateRoutine = ({ route, navigation }) => {
    ...
    useEffect(() => {
        const fetchRoutine = async () => {
            try {
                const data = await fetchData(`https://ama-backend-rbqv.onrender.com/api/routine/${itemId}`);
                setInitialData({
                    title: data.title,
    ...
    const handleSave = async (title, start_time, end_time, description) => {
        try {
            await fetchData(`https://ama-backend-rbqv.onrender.com/api/routine/${itemId}`, 'PUT', { title, start_time, end_time, description });
