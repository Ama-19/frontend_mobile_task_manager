...
export default function UpdateTask({ route, navigation }) {
    ...
    useEffect(() => {
        const loadTask = async () => {
            try {
                const response = await fetchData(`https://ama-backend-rbqv.onrender.com/api/task/${itemId}`, 'GET');
                setInitialData({
    ...
    const handleSave = async (title, order, description) => {
        try {
            await fetchData(`https://ama-backend-rbqv.onrender.com/api/task/${itemId}`, 'PUT', { title, priority: order, description });
